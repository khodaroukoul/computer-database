...
<header class="navbar navbar-inverse navbar-fixed-top">
    ...
    <div>
        <a href="addUser">
            <i class="fa fa-user" style="font-size:28px;color:#de4e4e"
               title="<spring:message code="label.addUser"/>">
            </i>
        </a>
        <a href="${pageContext.request.contextPath}/logout">
            <i class="fa fa-sign-out" style="font-size:28px;color:#de4e4e;margin-left: 10px;"
               aria-hidden="true" title="<spring:message code="label.logout"/>">
            </i>
        </a>
    </div>
</header>
            ...
            <div class="pull-left">
                <form id="searchForm" action="#" method="GET" class="form-inline">
                    <input type="search" id="searchbox" name="search"
                        ...
                        class="btn btn-primary" style="margin-left: 3px;"/>
                </form>
            </div>

            <div class="pull-left" style="margin-left: 50px;">
                <form id="deleteCompanyForm" action="dashboard/deleteCompany" method="POST"
                      class="form-inline">
                    <input type="search" id="deleteCompanyBox" name="deleteCompany"
                           class="form-control"
                           placeholder="<spring:message code="label.searchCompanyId" />"/>
                    <input type="submit" id="deleteCompanySubmit"
        ...
        <table class="table table-striped table-bordered">
            ...
            <tr>
                ...
                <th class="editMode" style="width: 60px; height: 22px;"><input
                        ...
                        style="vertical-align: top;"> - <a href="#" id="deleteSelected"
                ...
                <th>
                    <a href="dashboard?order=computer&search=${search}&currentPage=${previousPage}&computersPerPage=${computersPerPage}"
                       onclick=""><spring:message code="label.computerName"/></a></th>
                <th>
                    <a href="dashboard?order=introduced&search=${search}&currentPage=${previousPage}&computersPerPage=${computersPerPage}"
                       onclick=""><spring:message code="label.dateIntro"/></a></th>
                <th>
                    <a href="dashboard?order=discontinued&search=${search}&currentPage=${previousPage}&computersPerPage=${computersPerPage}"
                       onclick=""><spring:message code="label.dateDisc"/></a></th>
                <th>
                    <a href="dashboard?order=company&search=${search}&currentPage=${previousPage}&computersPerPage=${computersPerPage}"
...
<footer class="navbar-fixed-bottom">
    <div class="container text-center">
        <ul class="pagination">
            <li>
                <a href="dashboard?currentPage=${previousPage}&computersPerPage=${computersPerPage}&search=${search}&order=${order}F"
                   aria-label="Previous"> <span aria-hidden="true">&laquo;</span>
                </a></li>

            <c:forEach begin="${pageBegin}" end="${pageEnd}" var="i">
                <li>
                    <a href="dashboard?currentPage=${i}&computersPerPage=${computersPerPage}&search=${search}&order=${order}">${i}</a>
                </li>
            </c:forEach>
            <li>
                <a href="dashboard?currentPage=${nextPage}&computersPerPage=${computersPerPage}&search=${search}&order=${order}"
